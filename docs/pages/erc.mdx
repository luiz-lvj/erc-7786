...
### Glossary of Terms
 - **Origin chain:** The chain from where the sender is sending the message
 - **Destination chain:** The chain where the message is received and executed
 - **Source gateway:** A contract on the source chain that implements the ERC (natively or as an adapter) in order to send messages to receivers on other chains.
 - **Destination gateway:** A contract on the destination chain that implements the ERC in order to receive messages from other chains.
 - **Receiver:** The contract responsible for receiving the message and executing it in the destination chain (implements `IERC7786Receiver`).
 - **CAIP-2:** Chain Agnostic Improvement Proposal that defines chain identifier format, i.e., chain ids.
 - **CAIP-10:** Chain Agnostic Improvement Proposal that defines **account** identifier format.
