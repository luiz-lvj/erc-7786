...
This is what `ERC7786Aggregator` brings to the table. The logic is similar to a multisig, where a threshold of signers must sign a transaction in order to execute it, in the aggregator the message sent from the source chain must be sucessfully sent through different gateways before being executed on the destination chain. The ERC7786 Aggregator is an open bridge, enforcing that an identical cross-chain message is confirmed through multiple bridge gateways before it can execute on the destination chain.
...
<img src="https://raw.githubusercontent.com/luiz-lvj/erc-7786/technical/docs/assets/image-openbridge.svg" alt="Aggregator diagram" width="100%" height="auto" />
